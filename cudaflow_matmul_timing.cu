...
int main() {
    ...
    auto cudaFlow = taskflow.emplace([&](){
        tf::cudaGraph cg;
        auto matmulkernel0 = cg.kernel(grid, block, 0, matmul0, da, db, dc, N, N, N);
        auto matmulkernel1 = cg.kernel(grid, block, 0, matmul1, da, db, dc, N, N, N);
        auto matmulkernel2 = cg.kernel(grid, block, 0, matmul2, da, db, dc, N, N, N);
        auto matmulkernel3 = cg.kernel(grid, block, 0, matmul3, da, db, dc, N, N, N);
        auto matmulkernel = cg.kernel(grid, block, 0, matmul, da, db, dc, N, N, N);

        matmulkernel1.succeed(matmulkernel0)
                     .precede(matmulkernel2);
        matmulkernel3.succeed(matmulkernel2)
